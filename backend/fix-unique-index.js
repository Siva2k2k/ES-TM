...
async function fixUniqueIndex() {
  try {
    ...
    if (indexes[problematicIndex]?.unique) {
      ...
      console.log(
        "This prevents creating new adjustments when soft-deleted records exist.\n"
      );

      console.log("SOLUTION OPTIONS:");
      console.log("1. Drop the unique index (recommended)");
      console.log(
        "2. Create a partial unique index with { deleted_at: null } filter"
      );
      ...
      try {
        ...
        console.log(
          "New adjustments for soft-deleted records can now be created."
        );
      } catch (dropError) {
