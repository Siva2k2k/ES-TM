  ...
  try {
    ...
    if (adjustments.length === 0) {
      ...
      for (const adj of adjustments) {
        ...
        console.log(
          `Period: ${
            adj.billing_period_start?.toISOString().split("T")[0]
          } to ${adj.billing_period_end?.toISOString().split("T")[0]}`
        );
        console.log(`Scope: ${adj.adjustment_scope}`);
        console.log("");
        console.log(`Total Worked Hours: ${adj.total_worked_hours}h`);
        console.log(
          `Adjustment Hours: ${adj.adjustment_hours > 0 ? "+" : ""}${
        ...
        console.log(
          `Adjusted At: ${adj.adjusted_at?.toISOString().split("T")[0]}`
        );
    ...
    const timesheetMgmt = await Project.findOne({
    ...
    if (johnDevH && timesheetMgmt) {
      ...
      if (specificAdj) {
        ...
        console.log(
          `  Adjustment: ${specificAdj.adjustment_hours > 0 ? "+" : ""}${
            specificAdj.adjustment_hours
          }h`
        );
        console.log(`  Final Billable: ${specificAdj.total_billable_hours}h`);
        console.log(
          `  Original Billable: ${specificAdj.original_billable_hours}h`
        );
        console.log(
          `  Adjusted Billable: ${specificAdj.adjusted_billable_hours}h`
        );
        console.log("");
        ...
        console.log(
          `Difference: ${68 - (specificAdj.adjusted_billable_hours || 0)}h`
        );
      } else {
