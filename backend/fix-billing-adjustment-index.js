...
async function fixBillingAdjustmentIndex() {
  try {
    ...
    const problematicIndex =
      "user_id_1_project_id_1_billing_period_start_1_billing_period_end_1";

    console.log("=== FIXING UNIQUE INDEX ISSUE ===\n");
    console.log(
      "Problem: Unique index prevents creating new adjustments when soft-deleted records exist"
    );
    console.log(
      "Solution: Drop the unique index and create a partial unique index\n"
    );
    ...
    console.log(
      "Step 2: Creating partial unique index (only for deleted_at: null)..."
    );
    try {
      ...
      console.log(
        "   Constraint: Unique for active records (deleted_at: null)"
      );
      console.log(
        "   Benefit: Soft-deleted records no longer block new adjustments\n"
      );
    } catch (createError) {
    ...
    const timesheetProject = await Project.findOne({
    ...
    if (john && timesheetProject) {
      ...
      console.log(
        "Attempting to insert test document for John + Timesheet Management..."
      );
