...
async function testBillingAggregation() {
  try {
    await mongoose.connect(
      process.env.MONGODB_URI || "mongodb://localhost:27017/es-tm"
    );
    ...
    timesheets.forEach((ts) => {
      console.log(
        `  - ${ts.user_id?.full_name || "Unknown"}: Week ${
    ...
    Object.keys(groupedByProject).forEach((projectName) => {
      ...
      groupedByProject[projectName].forEach((approval) => {
        ...
        const weekStart =
          approval.timesheet_id?.week_start_date?.toISOString().split("T")[0] ||
          "N/A";
        ...
        console.log(
          `      Worked: ${approval.worked_hours}h, Adjustment: ${approval.billable_adjustment}h, Billable: ${approval.billable_hours}h`
        );
      });
    });
    console.log("");
    ...
    const approvedApprovals = allApprovals.filter(
      (approval) =>
    ...
    console.log(
      `Found ${approvedApprovals.length} management-approved records`
    );
    ...
    approvedApprovals.forEach((approval) => {
      ...
      approvedGrouped[projectName].users[userName].worked +=
        approval.worked_hours || 0;
      approvedGrouped[projectName].users[userName].adjustment +=
        approval.billable_adjustment || 0;
      approvedGrouped[projectName].users[userName].billable +=
        approval.billable_hours || 0;

      approvedGrouped[projectName].totalWorked += approval.worked_hours || 0;
      approvedGrouped[projectName].totalAdjustment +=
        approval.billable_adjustment || 0;
      approvedGrouped[projectName].totalBillable +=
        approval.billable_hours || 0;
    });
    ...
    Object.keys(approvedGrouped).forEach((projectName) => {
      ...
      Object.keys(projectData.users).forEach((userName) => {
        ...
        console.log(
          `      Adjustment: ${userData.adjustment > 0 ? "+" : ""}${
            userData.adjustment
          }h`
        );
      ...
      console.log(
        `    TOTAL Adjustment: ${projectData.totalAdjustment > 0 ? "+" : ""}${
          projectData.totalAdjustment
        }h`
      );
    ...
    if (approvedGrouped[expectedProject]) {
      ...
      console.log(
        `  Expected Worked: 288h, Actual: ${data.totalWorked}h ${
          data.totalWorked === 288 ? "✓" : "✗"
        }`
      );
      console.log(
        `  Expected Adjustment: +4h, Actual: ${
          data.totalAdjustment > 0 ? "+" : ""
        }${data.totalAdjustment}h ${data.totalAdjustment === 4 ? "✓" : "✗"}`
      );
      console.log(
        `  Expected Billable: 292h, Actual: ${data.totalBillable}h ${
          data.totalBillable === 292 ? "✓" : "✗"
        }`
      );
      ...
      Object.keys(expectations).forEach((userName) => {
        if (data.users[userName]) {
          ...
          const match =
            actual.worked === expected.worked &&
            actual.adjustment === expected.adjustment &&
            actual.billable === expected.billable;
          console.log(`    ${userName}: ${match ? "✓" : "✗"}`);
          if (!match) {
            console.log(
              `      Expected: ${expected.worked}h + ${expected.adjustment}h = ${expected.billable}h`
            );
            console.log(
              `      Actual:   ${actual.worked}h + ${actual.adjustment}h = ${actual.billable}h`
            );
          }
        } else {
